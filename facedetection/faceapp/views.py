...
@api_view(['get'])
def faceapp_view(request):
    
    if request.method == 'GET':
        # Load trained model
        face_recognizer = cv2.face.LBPHFaceRecognizer_create()
        face_recognizer.read('faceapp/trained_model.xml')

        face_cascade = cv2.CascadeClassifier('faceapp/haarcascade_frontalface_default.xml')
        cap = cv2.VideoCapture(0)

        #file for storing
        f = open('faceapp/emotion_labels.txt', 'a')
